...
def get_train_loaders(config: dict) -> dict[str, DataLoader]:
    ...
    # pin_memory: faster CPU→GPU transfers via pinned (page-locked) memory
    # persistent_workers: keep worker processes alive between epochs (avoid respawn overhead)
    # prefetch_factor: pre-fetch more batches to hide augmentation latency (ElasticDeformation ~1s/patch)
    pin = device == TorchDevice.CUDA
    pw = num_workers > 0
