...
def create_trainer(config: dict) -> "UNetTrainer":
    ...
    eval_cfg = trainer_config.pop("evaluation", None)
...
class UNetTrainer:
    ...
    def __init__(
        ...
        # AMP: enabled on CUDA only (RTX 4060 supports BF16 — compute cap 8.9)
        self._amp_enabled = (device == TorchDevice.CUDA)
        self._scaler = GradScaler(device="cuda") if self._amp_enabled else None

        if resume is not None:
            logger.info(f"Loading checkpoint '{resume}'...")
            # Preserve the LR from config — load_checkpoint overwrites optimizer
            # state_dict (including LR) with the saved checkpoint value.
            configured_lr = self.optimizer.param_groups[0]["lr"]
            ...
            logger.info(
                ...
                f"Best val score: {state['best_eval_score']}. LR overridden to {configured_lr} (from config)."
            )
    ...
    def _run_post_training_evaluation(self):
        """Run comprehensive evaluation on the test set after training completes.

        Activated when 'evaluation' key is present in the trainer config block.
        Expected YAML structure under trainer.evaluation:
          evaluation:
            pred_dir:    path/to/prediction_h5_files   # from StandardPredictor
            gt_dir:      path/to/original_test_h5s
            num_classes: 5
            output_dir:  path/to/eval_results/
            class_names: [Background, Segment_1, Segment_2, Segment_3, Segment_4]  # optional

        If 'evaluation' is absent the method is a no-op (backwards compatible).
        """
        eval_cfg = getattr(self, "_eval_cfg", None)
        if eval_cfg is None:
            return
        ...
        try:
            ...
            run_evaluation(
                pred_dir=eval_cfg["pred_dir"],
                gt_dir=eval_cfg["gt_dir"],
                num_classes=eval_cfg.get("num_classes", 5),
                output_dir=eval_cfg.get("output_dir", os.path.join(self.checkpoint_dir, "eval")),
                class_names=eval_cfg.get("class_names", None),
                pred_suffix=eval_cfg.get("pred_suffix", "_predictions"),
                pred_dataset=eval_cfg.get("pred_dataset", "predictions"),
                gt_dataset=eval_cfg.get("gt_dataset", "label"),
                num_qual_volumes=eval_cfg.get("num_qual_volumes", 3),
            )
        except Exception as exc:
            logger.error(f"Post-training evaluation failed: {exc}", exc_info=True)
    ...
    def train(self):
        ...
        for t in self.loaders["train"]:
            ...
            with autocast(device_type="cuda", enabled=self._amp_enabled):
    ...
    def should_stop(self):
        ...
        if self.early_stopping_patience is not None and self._no_improvement_count >= self.early_stopping_patience:
            logger.info(
                f"Early stopping: val score did not improve for {self._no_improvement_count} consecutive validations."
            )
            return True
