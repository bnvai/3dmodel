...
class WeightedCrossEntropyLoss(nn.Module):
    ...
    def _class_weights(input, target):
        ...
        target_flat = target.view(-1).long().clamp(min=0)
        counts = torch.zeros(n_classes, device=input.device, dtype=input.dtype)
        for c in range(n_classes):
            counts[c] = (target_flat == c).float().sum()
        total = counts.sum().clamp(min=1.0)
        # present classes: inverse frequency; absent classes: weight=1 (neutral)
        weights = torch.where(
            counts > 0,
            (total - counts) / counts.clamp(min=1.0),
            torch.ones(n_classes, device=input.device, dtype=input.dtype),
        )
        # clamp min=0.1 so dominant class always has non-zero weight
        # (avoids NaN when F.cross_entropy normalises by sum of per-pixel weights)
        return weights.clamp(min=0.1, max=100.0).detach()


class WCEDiceLoss(nn.Module):
    """Combined Weighted Cross-Entropy + Soft Dice for multi-class segmentation with integer labels.

    loss = alpha * WCE(logits, target_int) + (1 - alpha) * SoftDice(softmax(logits), target_onehot)

    WCE handles global class imbalance via dynamic weighting; Dice directly optimises
    per-class overlap and is especially effective for small rare structures
    (wiring, solder joints).  Integer labels are one-hot encoded internally for Dice.

    Args:
        alpha: Weight for WCE component (0..1). Default 0.5.
        ignore_index: Class index to exclude from WCE. Default -1.
    """

    def __init__(self, alpha=0.5, ignore_index=-1):
        super().__init__()
        self.alpha = alpha
        self.wce = WeightedCrossEntropyLoss(ignore_index=ignore_index)
    ...
    def forward(self, input, target):
        ...
        target_onehot = F.one_hot(target.long().clamp(min=0), num_classes=num_classes)
...
def get_loss_criterion(config):
    ...
    if not (ignore_index is None or name in ["CrossEntropyLoss", "WeightedCrossEntropyLoss", "WCEDiceLoss"]):
